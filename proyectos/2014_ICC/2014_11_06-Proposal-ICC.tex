...
dynamics simulations \cite{Nitsche2014} \todo{poner referencias del grupo}.
...
To perform numerical simulations in Chemistry and Biochemistry, there are two main families of models that differ in the level of detail.
...
On one hand, we can find the models based on the principles of Quantum Mechanics (QM), which allow obtaining a detailed description of the electronic and molecular structures of the systems under study.
The high computational demand associated with the usage of these models, restricts the size of the systems that can be analyzed.
...
Due to these restrictions, a family of models based on Molecular Mechanics (MM) emerged. 
In this kind of models, the atoms are represented using charged spheres and the chemical unions using springs, avoiding the details of electronic structure.
...
These simplified models decrease the associated computational cost notoriously, but can not describe reactive processes in which the changes in the electronic structure is important (for example in a chemical reaction and interaction with light).
...
The study of reactive phenomena in Chemistry, Biochemistry and Material Science requires modelling of large systems (including thousands of atoms) which can not be analyzed using QM models due to their size, but MM models become useless because of their intrisic reactive processes.
In this context, hybrid QM/MM methods emerged. 
In this kind of models, the reactive portion of the system is studied using a QM model and the rest of it with a MM model.
...
In this way, large systems in which the electronic structure plays a fundamental rol can be studied with a reasonable associated computational cost.
...
Despite the reduction of the system size to be analyzed using QM models, the total computational cost still is dominated by this part of the system.
Consequently, results higly relevant the reduction and optimization of the execution times of QM models to broaden the range of systems that can be studied in a reasonable time.
...
LIO is designed to be used as standalone application or as a library to be combined with other Molecular Dynamics package to create a QM/MM simulation tool.
This versatilty constitutes a design advantage to broaden its possibilities of use.
...
\texttt{Sander} and LIO are combined to create a powerful open source QM/MM simulation tool.\todo{quizas hace falta mencionar algo de que estan perfectamente acoplados}
...
As was already mentioned, the integration LIO/Sander is validated as can be seen in literature \todo{poner citas de amber y lio}. 
...
% For a complete and detailed description of these topics, the book by Levine~\cite{Levine2013} should be considered. 
