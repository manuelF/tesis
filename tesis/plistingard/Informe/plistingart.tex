...
\vspace{10mm}
\Huge\textbf{Computaci\'on de alto rendimiento en el estudio de reacciones qu\'imicas en soluci\'on a trav\'es de m\'etodos h\'ibridos cl\'asico-cu\'anticos.}
\vspace{10mm}
}
...
Usualmente, se cuenta con un estado inicial, y el problema consiste en ir determinando los estados subsiguientes, para observar c\'omo interact\'uan estos cuerpos en el tiempo.
...
En el caso de estudio de dos cuerpos celestes que se encuentran bajo acci\'on gravitatoria, si se considera que el centro com\'un de masa de los dos cuerpos esta en reposo, cada cuerpo viaja en una secci\'on c\'onica cuyo foco est\'a en el centro de masa del sistema.
...
Simular reacciones qu\'imicas en soluci\'on requiere la representaci\'on de un sistema con un gran n\'umero de part\'iculas.
En nuestro caso, como se ver\'a  m\'as adelante, se han realizado pruebas con alrededor de 500 mol\'eculas.
...
Por lo tanto, lo que se busca es hacer uso de las herramientas de mec\'anica cu\'antica en una regi\'on limitada para modelar el problema, mientras que el resto es modelado mediante mec\'anica molecular (ver figura \ref{fig:Hibrido}) para reducir su complejidad. La base de esta t\'ecnica consiste en combinar c\'alculos de estructura electr\'onica con la mec\'anica molecular cl\'asica a trav\'es de un Hamiltoniano h\'ibrido ($H_{qm-mm}$).
...
\begin{figure}[ht!]
\centering
\includegraphics[keepaspectratio, width=14cm]{ProfilingColor}
...
La paralelizaci\'on del algoritmo se realiza a trav\'es de la descomposici\'on del dominio de la integral presente en el tercer t\'ermino de la ecuaci\'on \ref{energia} (p\'agina \pageref{energia}).
...
En esa figura se ve como, en vez de realizar la sumatoria del vector RMM original m\'as los siete \'atomos se toman dos conjuntos, se realizan las sumatorias parciales y luego se suman los resultados para obtener el valor final.
...
Uno de los primeros problemas surgidos durante la implementaci\'on del software paralelo fue la configuraci\'on y utilizaci\'on de la librer\'ia \texttt{MPI} bajo Fortran.
En el archivo main.f se agregaron las instrucciones de inicializaci\'on de \texttt{MPI}, sin embargo, en las subrutinas del programa las contantes definidas en la librer\'ia \texttt{MPIF} no eran reconocidas, lo cu\'al produc\'ia errores al ejecutar el software.

Un ejemplo de este caso se produjo con el comunicador \texttt{MPI\_COMM\_WORLD} que se utiliza para enviar mensajes a todos los procesos involucrados en la ejecuci\'on.
...
La instrucci\'on \texttt{MPI\_Bcast} realiza una comunicaci\'on colectiva donde un proceso env\'ia los mismos datos a todos los procesos presentes en el comunicador.
...
Esto pudo ser observado por medio de la realizaci\'on de una rutina que permite guardar mediante un archivo de texto una cantidad de variables que cre\'imos convenientes analizar para determinar la correctitud de los c\'alculos, lo que denominaremos como \emph{dump}  o \emph{estado} del programa.

En un principio se pens\'o que el problema era debido a un error en la manera de realizar el c\'alculo, pero luego de un an\'alisis se confirm\'o que el mismo era correcto.
...
Luego se realiz\'o la corrida con los 4 \'atomos pero solo guardando el resultado de los \'atomos 3 y 4.
...
Una vez reducido el \'area donde pod\'ia ocurrir el problema se procedieron a realizar otros tests para determinar finalmente el problema. Para tal efecto se realiz\'o una modificacion al programa serial para que calcule en RMM el resultado de los 2 primeros \'atomos, y los \'ultimos dos los guarde en un acumulador. Finalmente se uni\'o el vector RMM con el acumulador por medio de sumas. Esto hizo que el programa serial diera el mismo resultado que el paralelo. De esta manera se demostr\'o que el problema se debe a la limitaci\'on generada por la finitud de los n\'umeros representados en la computadora, lo cual es un problema ampliamente conocido en los m\'etodos num\'ericos. Ver figura \ref{fig:Redondeo}
...
\end{center}
\caption[Speedup del programa con reordenamiento de c\'alculo en dos procesos en una m\'aquina dual con una entrada de \chemcom{OONO^- + CO_2}.]{\small{Speedup del programa con reordenamiento de c\'alculo en 2 procesos en una m\'aquina SMP Xeon Dual (Papita) con una entrada de \chemcom{OONO^- + CO_2}.}}
\label{fig:Speedup}
\end{figure}
...
\end{center}
\caption[Eficiencia del programa con reordenamiento de c\'alculo en dos procesos en una m\'aquina dual con una entrada de \chemcom{OONO^- + CO_2}.]{\small{Eficiencia del programa con reordenamiento de c\'alculo en 2 procesos en una m\'aquina SMP Xeon Dual (Papita) con una entrada de \chemcom{OONO^- + CO_2}.}}
...
\end{center}
\caption[Overhead del programa con reordenamiento de c\'alculo en dos procesos en una m\'aquina dual con una entrada de \chemcom{OONO^- + CO_2}.]{\small{Overhead del programa con reordenamiento de c\'alculo en 2 procesos en una m\'aquina SMP Xeon Dual (Papita) con una entrada de \chemcom{OONO^- + CO_2}.}}
\label{fig:Overhead}
\end{figure}
...
Luego fueron realizadas mediciones en el cluster \emph{FIUPG}, sito en el estado de Florida, Estados Unidos.
...
No fueron utilizados m\'as procesadores del cluster debido a que con el tama\ ~no de las entradas con las que se realizaron las pruebas no permite exigir ejecuciones con un n\'umero de procesos mayor.
...
\end{tabular}
\end{center}
\caption[Tiempos de ejecuci\'on total del programa serial, y el paralelo en 2 y 4 procesos con una entrada de de \chemcom{OONO^- + CO_2}]{\small{Tiempos de ejecuci\'on total del programa serial, y el paralelo en 2 y 4 procesos con una entrada de \chemcom{OONO^- + CO_2}.}}
...
\end{center}
\caption[Tiempos de ejecuci\'on total del programa en un cluster con una entrada de \chemcom{OONO^- + CO_2}.]{\small{Tiempos de ejecuci\'on total del programa serial, y el paralelo en 2 y 4 procesos con una entrada de \chemcom{OONO^- + CO_2}.}}
...
En contraparte a los resultados obtenidos puede observarse el gran overhead introducido en la versi\'on paralela del programa debido al pasaje de mensajes, lo cual hace que el \emph{Speedup} no sea lineal con respecto a la cantidad de procesos utilizados. Esto quiere decir que si el \emph{Speedup} fuera lineal, utilizando dos procesos el c\'alculo deber\'ia tardar la mitad de tiempo.
...
Cada una de las formas en que vibra una mol\'ecula es llamada \emph{modo normal}.
En \'estas figuras pueden observarse las frecuencias correspondientes a cada modo normal de la mol\'ecula de peroxinitrito en solvente.
En ambas figuras se aprecia un marcado ensanchamiento en la banda correspondiente al estiramiento \chemcom{N-O_3}, indicando un fuerte efecto solvente para este modo en particular.
Estos resultados obtenidos coinciden con valores experimentales obtenidos\cite{Lebrero2005} y mediante este trabajo se pudo reinterpretar los mismos. Los errores generales son peque\~nos as\'i como tambi\'en los errores con respecto a la sensible banda de estiramiento \chemcom{N-O_3}.
El ensanchamiento y el solvente expl\'icito no pueden tratarse con los m\'etodos tradicionales de simulaci\'on de estos espectros.

Este estudio sirvi\'o para reinterpretar el espectro experimental originalmente mal asignado, as\'i como para dar
...
A pesar de que los autores originales del programa ten\'ian la noci\'on de existencia de estos problemas nunca se lo hab\'ia demostrado de forma tan contundente, lo cual fue un avance para la toma de conciencia y un planteo sobre la forma de trabajo y el rumbo a seguir.
...
La primera versi\'on realizada alcanz\'o los objetivos en cuanto a correctitud de los resultados y estabilidad num\'erica, sin embargo al trabajar con n\'umeros de punto flotante de precisi\'on cu\'adruple la performance del programa se vi\'o afectada de forma notable.
...
Con la implementaci\'on que finalmente se utiliz\'o en este estudio se logr\'o una aproximaci\'on al programa serial que aunque no otorga resultados exactamente iguales, aunque fueron tomados como v\'alidos por los expertos del \'area.

En esta implementaci\'on se opt\'o por ocasionar una p\'erdida de precisi\'on para poder estudiar sistemas con un programa paralelo.
Este programa abre un abanico de posibilidades para poder resolver sistemas intratables hasta el momento, he aqu\'i su mejor caracter\'istica.
...
Como pudo observarse, el algoritmo paralelo se vuelve m\'as eficiente conforme el dominio del problema crece. Esto se debe a que el overhead producido por la comunicaci\'on entre procesos se ve compensado por la cantidad de instrucciones ejecutadas en cada uno de los nodos.

Para diversas corridas nuevas se utiliz\'o una SMP Xeon Dual debido a que la ganancia de la misma era de un $15\%$ aproximadamente en el caso de Papita, y alrededor de un $50\%$ (esto debido a que el cluster \emph{FIUPG} fue utilizado en exclusividad y posee m\'aquinas mas potentes, y Papita era utilizada aleatoriamente por otros usuarios) en el caso de cada nodo del cluster \emph{FIUPG}, lo cual reduc\'ia los tiempos de cada corrida de forma notoria.
Se opt\'o por este tipo de m\'aquina porque los costos de comunicaci\'on mediante buses externos afectan la performance.
