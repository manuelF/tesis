...
Para resolver el primer punto, utilizamos una matriz de Fock y una matriz de
fuerzas distinta para cada hilo de ejecuci\'on, de manera que sus grupos asignados
usaran esta matrices en vez de las globales. Al finalizar la iteraci\'on, todas
las contribuciones de estas matrices son devueltas a la matriz global una a la
...
Para la segunda parte, hicimos un an\'alisis del c\'odigo de la iteraci\'on para
reestructurarlo para que fuese ameno a la paralelizaci\'on mediante OpenMP.
...
utilizamos un \textit{benchmark} dise\~nado para ilustrar la falta
...
\begin{figure}[htbp]
    ...
    \end{lstlisting}
    \caption{Ciclo principal de la implementacion de un algoritmo de suma paralelo de matrices.
    El codigo para \texttt{sum}, que suma una matriz con la otra, se encuentra en el apendice.}
\end{figure}
%TODO(jpdarago): Poner este codigo en un apendice
Claramente este algoritmo debiera producir un mejor tiempo de ejecuci\'on, en
teor\'ia, utilizando multiples procesadores. Esto suena razonable considerando
...
lecturas de memoria y operaciones con los datos leidos es por lo tanto muy baja.

Observemos adem\'as que, al no utilizarse los datos multiples veces, cada l\'inea
de datos tiene un \textit{cache miss} asociado (el de la l\'inea donde reside) que
no se ve compensado por uso del mismo mientras reside en cache.
...
\textit{performance} que presentamos en las secciones anteriores, para dar estad\'
sticas a nivel arquitectural de la aplicaci\'on y de muy fino detalle, como por
...
un an\'alisis de \textit{perf} para 1 y 12 hilos simultaneos en la m\'aquina de
...
Esto nos lleva a concluir que esta operaci\'on final impactar\'ia muy
...
c\'odigo se har\'ia mucho m\'as pesada. Por lo tanto, buscamos reorganizar
este ciclo para no necesitar una matriz separada.

El ciclo a modificar esta en la figura~\ref{fig:rmm.output-previous}.
Se ve f\'acilmente que esto es posible si invertimos los ciclos del algoritmo en
el algoritmo, podemos dividir los \'indices de la matriz global entre multiples
hilos de ejecuci\'on.

\begin{algorithm}[H]
    \centering
    \label{algo:rmm-output-previous}
    \caption{C\'alculo original de la matriz de Fock}
    \begin{algorithmic}
        \State $R \gets 0_{m,n}$
        \State $F \gets functions(PG)$
...
\begin{algorithm}[H]
    ...
    \caption{C\'alculo de la matriz de Fock reestructurado para paralelismo}
    \begin{algorithmic}
        \State $R \gets 0_{m,n}$
...
\begin{figure}[htbp]
   \centering
   \includegraphics[width=\plotwidth]{plots/cpu/histogram-functions-hemo.png}
...
una cantidad de puntos y una cantidad de \'indices en la matriz de Fock mayor que
...
no negligible en algunos casos, con lo cual todav\'ia se puede obtener algo de
mejoras en \textit{performance} entre iteraciones utilizando balanceo de cargas.

Para aliviar esta situaci\'on ligeramente, introdujimos un algoritmo de balanceo
de cargas similar al utilizado para distribuir cargas en GPU, usando pasaje de
trabajos. Un esquema del mismo puede verse en el pseudoc\'odigo de la figura
\ref{algo:algo-work-stealing-cpu}.

El algoritmo es deliberadamente simple, para tener poco \textit{overhead}. Se
mide el costo de cada grupo y luego se pasa el trabajo de menor costo (seg\'un
nuestra estimaci\'on) del grupo m\'as pesado (el que termino \'ultimo) al
m\'as ligero (el que termino primero). Esto solo se realiza si la diferencia
de tiempos entre la carga m\'as pesada y la m\'as ligera es de m\'as del 5\%.

Al elegir el grupo m\'as chico nos aseguramos que no movemos un grupo muy
grande por error, potencialmente desbalanceando la partici\'on . La desventaja
es que la convergencia es mucho m\'as lenta, pero esto no es demasiado
problema puesto que como ya mencionamos, las diferencias de cargas
experimentalmente resultan peque\~nas.

\begin{algorithm}[H]
    \caption{Pseudoc\'odigo para algoritmo de robo de trabajos}
    \label{algo:algo-work-stealing-cpu}
    \begin{algorithmic}
        \Function{rebalance}{$T : Times, W : Workload$}
            \State $largest \gets \arg\max T$
            \State $smallest \gets \arg\min T$
            \If{$\frac{T[largest] - T[smallest]}{T[largest]} > 0.05$}
                \State $minwork \gets \arg\min W[largest]$
                \State mover $minwork$ de $smallest$ a $largest$
            \EndIf
        \EndFunction
    \end{algorithmic}
\end{algorithm}

El costo computacional resulta bajo pues requiere de una iteraci\'on para cada
grupo, y no requiere medir granularmente (es decir, para cada grupo), aunque si
para cada carga de un hilo de ejecuci\'on.
