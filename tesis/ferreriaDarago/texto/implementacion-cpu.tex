...
\begin{algorithm}[hptb]
    ...
    \begin{algorithmic}
        ...
        \While{$\neg convergio(MatKohnSham)$}
            \ForAll{$Grupo \in Grupos(Particion)$}
            ...
            \Function{solve}{$PG : PointGroup$}
              ...
              \ForAll{$p \in puntos(PG)$}
                  ...
                  \ForAll{$i \in functions(PG, p)$}
                       \State Calcular la contribución a la densidad, $dens$
                       \State Calcular la contribución al gradiente, $grad$
                       \State Calcular la contribución a los hessianos, $hess_1$ y $hess_2$
                  \EndFor
              ...
              \State Sumar la contribución de cada punto a la matriz de Kohn Sham General
            \EndFunction
        \end{algorithmic}
\end{algorithm}
...
\begin{figure}[htbp]
   ...
   \caption{Divisi\'on de los tiempos de iteración para la hemoglobina en la versión
   original en CPU}
...
Queda pendiente como realizar la divisi\'on de trabajo.  En un primer momento se plantearon dos posibles rutas
