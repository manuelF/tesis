...
correcci\'on de tiempos de ejecuci\'on con los $k^2$ los coeficientes de correcci\'on para poder estimar el tiempo de
...
\begin{algorithmic}
  ...
  \While {$tiempo_{\max} < tiempo_{\min} * k$}
    ...
    \ForAll{$G_i \in Trabajos[T_{\max}]$}
      \If{$duracion[G_i] * correccion[T_{\max}][T_{\min}] * penalidad\_migracion - \Delta T < duracion_{G_{mejor\_candidato}}$}
