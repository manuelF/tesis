...
La subsaturaci\'on de los SM se da en los casos donde haya SM que est\'en listos para correr
...
va a correr sin desalojo hasta que terminen todos sus threads asignados. Idealmente, cada
...
register file por SM, com\'un a todos los bloques. Como cada thread usa decenas de registros para guardar
...
no pueda ejecutar concurrentemente m\'as bloques que los que pueda soportar este recurso, dejando SPs ociosos.
...
del grupo a resolver instanciando un bloque por cada punto ({$blockId.x < puntos$})
...
menos trabajo por llamada, de modo que se puedan despachar a m\'as SM a medida
...
La otra modificaci\'on importante fue partir en problema en m\'as bloques para las grupos que tengan
...
todas las funciones de ese punto. Se denomina este par\'ametro $altura\_bloques$ y se
...
grandes (de los sistemas probados), la altura puede ser hasta 6. Esto representa una gran
cantidad de bloques adicionales con respecto al m\'etodo anterior.

\begin{figure}[htbp]
   \centering
   \includegraphics[width=\plotwidth]{plots/cuda/threading.png}
   \caption{Aceleraci\'on en veces de correr Hemoglobina en distintas arquitecturas con el cambio de estrategia. Otros casos
   de prueba exhiben comportamientos similares.}
   \label{fig:threading}
\end{figure}
...
Para nuestro problema, se decidi\'o utilizar un numero de threads por bloque m\'ultiplo del
...
sincronizar nada puesto que en CUDA los threads trabajan en lock-step sincronizados por warp.
...
Por otra parte, la literatura~\cite{farberCuda} sugiere siempre que sea posible
usar bloques grandes y con threads lo m\'as independientes posibles. Una gran cantidad de threads
en un bloque permite tener muchos m\'as warps para ejecutar de modo de esconder las latencias de
operaciones y de a accesos globales. Sin embargo, contar con muchos threads hace que las
sincronizaciones sean mucho m\'as costosas. Ademas, como cada SM no cuenta con desalojo
...
muchos m\'as bloques. Finalmente, luego de disminuir un poco el uso de registros por thread mediante
un reorden de las operaciones, se fij\'o este valor en 64 threads
por bloque. Se muestra en la figura \ref{fig:dbs-runtime} que tener solo un warp es bueno, pero
mejor a\'un es tener dos warps ejecutando simult\'aneamente, ya que
esto permite, con costo cero, hacer un cambio de contexto al otro warp para ocultar la latencia sin agrandar
demasiado el costo de la sincronizaci\'on.

Habiendo hechas todas las dem\'as optimizaciones detalladas en las secciones siguientes, se evalu\'o
\'unicamente este par\'ametro. Se estudiaron bloques de 32, 64 y 128 threads por bloque,
el m\'aximo valor posible todav\'ia usando el modelo de memoria compartida.
...
suma de todos los elementos de la columna que se acaba de procesar para obtenerlo. Para reducir,
se reutiliza la memoria shared que fue empleada en el c\'alculo de la densidad anterior. Cada
...
hacer falta guardar estas cuentas parciales en memoria global. Para eso se defini\'o una matriz
por cada uno de los par\'ametros que se deben acumular, con tama\~no id\'entico a la cantidad de bloques
...
La arquitectura de las placas de video est\'an pensadas entorno al poder de computo.
...
no suced\'ia, ya que todos los threads realizaban cuentas para puntos independientes y no se compart\'ian las
lecturas de las matrices. Esto aprovechaba las dimensiones originales de las matrices y hac\'ian que las lecturas de
...
\ref{fig:transpose} se puede observar que tanto en Fermi como en Kepler es muy notoria la diferencia
...
\begin{figure}[htbp]
   ...
   \caption{Aceleraci\'on en veces del kernel de calculo de densidad corriendo Hemoglobina en distintas arquitecturas habiendo transpuesto las matrices
...
aplicaciones de render de video. Los mapas de textura suelen ser grandes matrices que definen
...
\begin{figure}[htbp]
   ...
   \caption{Aceleraci\'on del c\'alculo de densidad corriendo Hemoglobina en Fermi y Kepler
   usando memoria de textura para leer la matriz densidad.}
   \label{fig:texture}
\end{figure}

Como se puede ver en la figura~\ref{fig:texture}, esta optimizaci\'on aprovecha este
recurso \'unico de GPU. Sin embargo, esto agrega un factor mas que se debe tener que tener en
...
Cuando se depende de sobremanera de la memoria de textura para esconder la latencia,
se presentan contenciones sobre el acceso a estas unidades. Esta cach\'e hay que usarla
solamente en los accesos mas costosos \'unicamente, ya que si se intentan pasar todos
...
Esta estrategia es claramente pr\'actica en CPU, puesto que se cuenta con mucha m\'as memoria
...
se desarroll\'o un m\'etodos para poder almacenar las matrices de los valores de funciones y sus derivadas
...
de la partici\'on), la constante es elevada. Se determin\'o entonces que es m\'as conveniente
...
Se evalu\'o el tiempo de ejecuci\'on de un sistema modelando un fullereno $C_{60}$, variando la
...
peso de los grupos chicos en el tiempo total del c\'alculo de las funciones. Se decidi\'o no intentar
...
capacidad de almacenamiento, un recurso que hasta recientemente venia siendo un limitante se puede convertir
en una aceleraci\'on notoria.
...
Otro de los problemas existentes del c\'odigo que se busc\'o atacar fue el mejor empleo de las
...
el uso de la memoria shared de modo que no se est\'en subutilizando los SM.

Una cosa que se prob\'o, con un grado de \'exito variable, fue disminuir el tama\~no de los vectores
donde se almacenan las derivadas direccionales. La manera de guardar las derivadas direccionales primeras
...
se dejaron como estaban sin modificaciones.
...
eliminarlo ahorra una operaci\'on de c\'alculo. Mas a\'un, se observa una disminuci\'on del 18\%
...
como con incluso un 18\% m\'as de operaciones de lecto-escritura, el tiempo de ejecuci\'on casi no varia. Por
...
de la densidad electr\'onica y del c\'alculo de la matriz de Kohn-Sham, se lleg\'o a un punto donde
no fue posible determinar mejoras significativas en el c\'alculo para reducir tiempos.
Se decidi\'o subir un nivel m\'as el paralelismo, de modo de poder solucionar m\'ultiples particiones
...
Para dividir el problema entre varios dispositivos se usa, al igual que en CPU, OpenMP. Se define
...
una instrucci\'on del driver de CUDA (\texttt{CudaSetDevice}) que permite que durante toda la vida del
...
esta definida para su dispositivo actual. Esto puede ser un problema si se quiere lograr comunicaciones entre placas,
...
Para distribuir las tareas se utilizaron dos t\'ecnicas combinadas, una para distribuir las
...
Para la distribuci\'on est\'atica, se usa como estimador del runtime el tama\~no
...
la cantidad de memoria necesaria para operar. Se compar\'o tambi\'en contra el predictor usado
...
Como se puede apreciar, se puede ver una relaci\'on entre  el tiempo de resoluci\'on con el tama\~no
de los elementos con los que trabaja. Esto da una pauta de la complejidad computacional del problema
que se esta resolviendo. Como al menos se necesita cada uno de los elementos de cada matriz de funciones
para resolver un punto de cada grupo. Al menos se deben hacer una lectura de cada matriz. Estas lecturas
son tan costosas que, incluso solo sabiendo el tama\~no de las matrices, se puede predecir aproximadamente
...
No se pudieron encontrar par\'ametros que permitan ajustarlo para que funcione en GPU. Como \nvidia{} no publica toda la informaci\'on
...
$A$ es distinto a resolverlo en el dispositivo $B$. Por este motivo, no se puede particionarlo completamente
de manera est\'aticamente
usando este algoritmo y se agrega usar una soluci\'on h\'ibrida est\'atica y din\'amica, reutilizando
...
Inicialmente se defini\'o un orden para realizar toda la resoluci\'on del sistema y
...
de los dispositivos como proxy del tiempo de ejecuci\'on, se aplica una ronda del algoritmo de balanceo din\'amico
para terminar de balancear est\'aticamente. Esto se puede hacer ya que, como se mostr\'o en la figura~\ref{fig:global-fullereno},
...
Se considera que realizar este c\'omputo da una evaluaci\'on
...
y tiene un uso total de la memoria shared. No se contabilizaron las copias de memoria desde y hacia la
...
dado que hay menor concurrencia sobre estas. Se puede observar en la figura \ref{fig:4placas-doble} una clara mejora en los tiempos
