...
c*****************dihedro!!!!
      ...
      elseif(UMBRE.EQ.3) THEN
       ...
       xmz=(x(2)-x(3))*(y(4)-y(3))-(y(2)-y(3))*(x(4)-x(3))
       ...
       dscalar=xnx*dmx+xmx*dnx+xny*dmy+xmy*dny+xnz*dmz+xmz*dnz
        ...
        if(j.eq.1) FX(natdh)=FX(natdh)+dtot*(dscalar*xm*xn-dmn*scal)
     >  /(xn*xm)**2
        if(j.eq.2) Fy(natdh)=Fy(natdh)+dtot*(dscalar*xm*xn-dmn*scal)
     >  /(xn*xm)**2
        if(j.eq.3) Fz(natdh)=Fz(natdh)+dtot*(dscalar*xm*xn-dmn*scal)
     >  /(xn*xm)**2
