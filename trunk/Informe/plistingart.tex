...
probabilidad de encontrar a un electr\'on en un lugar dado del espacio.
...
Un gran n\'umero de las reacciones qu\'imicas de inter\'es ocurren en soluci\'on acuosa. Dentro de los ingredientes utilizados para modelar al agua se encuentran cargas puntuales fijas, cargas fluctuantes, dipolos puntuales, potenciales tipo Lennard-Jones, potenciales arm\'onicos de enlace y otros. La utilizaci\'n de dichos ingredientes, as\'i como la parametrizaci\'on de los mismos, dependen en buena medida de qu\'e propiedad interesa reproducir y con qu\'e nivel de precisi\'on en relaci\'on al costo computacional asociado \cite{Leach96}.

Los modelos de agua utilizados por el sistema serial son el TIP4P y el TIP4P-FQ.

\begin{itemize}

\item En el modelo TIP4P se representa a la mol\'ecula de agua como un cuerpo r\'igido (sin grados de libertad internos) con cuatro centros, uno centrado en cada \'atomo y uno extra en el eje $C_{2v}$ situado en la bisectriz del \'angulo H-O-H (ver figura \ref{fig:TIP4P}). Las mol\'eculas de agua TIP4P interact\'uan entre s\'i mediante interacciones Coul\'ombicas entre cargas situadas en los hidr\'ogenos y el sitio \texttt{M} y mediante un potencial de tipo Lennard-Jones centrado en el \'atomo de ox\'igeno.

\item En el modelo TIP4P-FQ, para incluir los efectos de la polarizaci\'on en un modelo de agua, se tiene en cuenta que el momento dipolar de cada mol\'ecula depende del campo el\'ectrico y \'este, a su vez, depende de los momentos dipolares. Para incluir estos efectos se realiza una din\'amica ficticia de las cargas.

\end{itemize}

\begin{figure}[ht!]
\centering
\includegraphics[keepaspectratio, width=8cm]{TIP4P}
\caption[Esquema de distribuci\'on de sitios para los modelos de
agua TIP4P y TIP4P-FQ]{\small{Esquema de distribuci\'on de sitios para los modelos de
agua TIP4P y TIP4P-FQ.}}
\label{fig:TIP4P}
\end{figure}
...
A estas metodolog\'ias h\'ibridas se les da el nombre QM/MM (Quantum Mecanics/molecular mecanics) \cite{Cramer,MCGLPHD}.
...
{En esta figura se muestra como en sistemas grandes la generaci\'on y ruptura de enlaces puede estudiarse mediante la representaci\'on cu\'antica de una peque\~na parte del mismo (Quantum Mechanics, QM), utilizando mec\'anica molecular (Molecular Mechanics, MM) para representar el resto. En este caso no hay relaci\'on entre los elementos de ambos subsistemas y el modelado es simple.}}
\label{fig:Hibrido}
\end{figure}
...
Donde $X_i^{KS}$ representa a el orbital $i$ de Kohn y Sham el cual est\'a expresado como combinaci\'on lineal de gaussianas \cite{Kohn65}, por lo que la densidad queda determinada por una sumatoria de productos de las mismas.
...
Finalmente la investigaci\'on correspondiente a la segunda entrada se realiza a efecto de estudiar como influye un solvente (agua) sobre el peroxinitrito. Pueden observarse dimeros utilizados para observar la correctitud de los c\'alculos de las fuerzas y la energ\'ia en la figura \ref{fig:peroxinitrito2}.
...
Finalmente, para demostrar la utilidad final del programa fueron realizadas corridas totalmente nuevas en \emph{FIUPG} y en la SMP Xeon Dual para contribuir con el procesamiento de datos a la investigaci\'on qu\'imica para la cual se hizo el programa serial. Estas corridas fueron desarrolladas por el grupo de qu\'imica inorg\'anica que provey\'o el programa serial y fueron supervisadas por los mismos mientras se realizaban para verificar la correctitud de los datos obtenidos. Mediante este tipo de corridas se contribuye a obtener informaci\'on sobre la interacci\'on molecular y obtener gr\'aficos como los de las figuras \ref{fig:Espectro1} y \ref{fig:Espectro2}
