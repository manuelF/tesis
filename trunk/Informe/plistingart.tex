...
\item {\it Memoria}: (0 avgtext + 0 avgdata 0 maxresident)k. Promedio de memoria total utilizada en el proceos dividida en datos, stack y texto.
...
Por ejemplo, al utilizar declaraci\'on impl\'icita de variables si el programador declara que las variables que empiecen con letras de la \texttt{A} a la \texttt{H} ser\'an del tipo \texttt{REAL*8} pero luego ante un descuido declara otra variable \texttt{INTEGER} con alg\'un nombre que empiece entre esas letras podr\'an darse errores o diferencias en los resultados.
...
Se utilizaron las librer\'ias \textsf{MPICH}\cite{MPICH} y \textsf{LAM MPI}\cite{LAM_MPI} para las pruebas realizadas.
...
Un ejemplo de este caso se produjo con el comunicador \texttt{MPI\_COMM\_WORLD} que incluye a todos los procesos ejecutandose en MPI.
...
Est\'o estaba lejos de ser lo \'optimo debido al overhead generado por el env\'io de mensajes.
...
Puesto que $\sqrt(3)$ no tiene una representaci\'on de d\'igitos finitos, en el interior de la computadora se le da una representaci\'on aproximada cuyo cuadrado no ser\'a exactamente $3$, aunque estar\'a lo bastante cerca a $3$ para resultar aceptable en la generalidad de los casos. Casi siempre la representaci\'on y aritm\'etica computacionales son satisfactorias
