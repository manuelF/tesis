...
\end{center}
\caption[Speedup del programa con reordenamiento de c\'alculo en 2 procesos en una m\'aquina SMP Xeon Dual (Papita) con una entrada de \chemcom{OONO^- + CO_2}.]{\small{Speedup del programa con reordenamiento de c\'alculo en 2 procesos en una m\'aquina SMP Xeon Dual (Papita) con una entrada de \chemcom{OONO^- + CO_2}.}}
%{\huge poner alg\'un dato de la corrida, recorda que la figura debe entederse solo con mirar el caption.}}
\label{fig:Speedup}
\end{figure}
...
\end{center}
\caption[Eficiencia del programa con reordenamiento de c\'alculo en 2 procesos en una m\'aquina SMP Xeon Dual (Papita) con una entrada de \chemcom{OONO^- + CO_2}.]{\small{Eficiencia del programa con reordenamiento de c\'alculo en 2 procesos en una m\'aquina SMP Xeon Dual (Papita) con una entrada de \chemcom{OONO^- + CO_2}.}}
%{\huge idem.}}
...
\end{center}
\caption[Overhead del programa con reordenamiento de c\'alculo en 2 procesos en una m\'aquina SMP Xeon Dual (Papita) con una entrada de \chemcom{OONO^- + CO_2}.]{\small{Overhead del programa con reordenamiento de c\'alculo en 2 procesos en una m\'aquina SMP Xeon Dual (Papita) con una entrada de \chemcom{OONO^- + CO_2}.}}
%{\huge idem.}}
\label{fig:Overhead}
\end{figure}
...
\end{tabular}
\end{center}
\caption[Tiempos de ejecuci\'on total del programa serial y el paralelo en 2 y 4 procesos con una entrada de 7 \'atomos]{\small{Tiempos de ejecuci\'on total del programa serial y el paralelo en 2 y 4 procesos con una entrada de \chemcom{OONO^- + CO_2}.}}
%{\huge aca hay tres filas, aunque el caption diga que hay dos cosas. Ademas podr\'ias poner qu\'e corriste.}}
...
%{\huge De nuevo, hay tres cosas graficadas... pone info de qu\'e corrida estas hablando.}}
...
Para diversas corridas nuevas se utiliz\'o una SMP Xeon Dual debido a que la ganancia de la misma era de un $15\%$ aproximadamente en el caso de Papita, y alrededor de un $50\%$ (esto debido a que el cluster \emph{FIUPG} fue utilizado en exclusividad y posee m\'aquinas mas potentes, y Papita era utilizada aleatoriamente por otros usuarios)
%{\huge estas seguro de esto, en que condiciones, aclara por que suena MUY raro que ande mejor que en una SMP} 
en el caso de cada nodo del cluster \emph{FIUPG}, lo cual reduc\'ia los tiempos de investigaci\'on de forma notoria. Se opt\'o por este tipo de m\'aquina porque los costos de comunicaci\'on mediante buses externos afectan la performance.
