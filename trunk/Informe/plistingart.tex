...
Con la primera entrada utilizada se intenta determinar el mecanismo de reacci\'on del peroxinitrito con dioxido de carbono. Existen dos principales razones por las cuales resulta interesante esta investigaci\'on, en primer lugar la alta concentración del \chemcom{CO_2} en medios fisiol\'ogicos y en segundo lugar la alta velocidad con que ocurre esta reacci\'on.
...
%{\huge Indicar que el overhead es sobre el tiempo total, ya que no coincide con la definicion dada anteriormente}
...
%{\huge Decir porque no podes usar los 17*2 procesadores que hay en el cluster y porque elegis correr un proceso por maquina.}
Las corridas fueron realizadas con dos y cuatro procesos en nodos distintos, es decir a raz\'on de un proceso en cada nodo, obteni\'endose los resultados que se pueden ver en la tabla \ref{tab:TiemposEjecucion3} y en la figura \ref{fig:FIUPGTiempos1}. No fueron utilizados mas procesadores del cluster debido a que cuanto mayor es la cantidad de procesos utilizados el error introducido por la aritm\'etica finita crece, de forma tal de hacer que los resultados obtenidos no sean utilizables. Por otra parte se eligi\'o utilizar nodos distintos para poder medir de mejor manera el overhead introducido por los buses externos de datos de baja velocidad.
...
Finalmente, para demostrar la utilidad final del programa fueron realizadas corridas totalmente nuevas en \emph{FIUPG} y en la SMP Xeon Dual. Los par\'ametros para estas corridas fueron preparados por el grupo de qu\'imica inorg\'anica que provey\'o el programa serial y fueron supervisadas por los mismos mientras se realizaban para verificar la correctitud de los datos obtenidos.
...
%{\huge todo muy lindo, pero no es posible dar un poco mas de informacion de estos graficos}
...
Cada una de las formas en que vibra una mol\'ecula es llamada modo normal. En \'estas figuras pueden observarse las frecuencias correspondientes a cada modo normal de la mol\'ecula de peroxinitrito en solvente.

%{\huge Si esto es trabajo a futuro, porque no lo pones en el capitulo correspondiente?}
...
Con la implementaci\'on que finalmente se utiliz\'o en este estudio se logr\'o una aproximaci\'on al sistema serial que aunque no otorga resultados exactamente iguales la perdida de precisi\'on es la misma y por lo tanto es aceptado por los expertos del \'area. 
%{\huge esto no se entiende que queres decir, la perdida de precisi\'on es la ``misma'' que la producida por la versi\'on serial, no?}
