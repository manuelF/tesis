...
{\Huge Idem arriba. Podes ponerle un label a la ecuacion y hacerle referencia aqui. Si queres dejar aca la parte del costo de calcularlo, me parece razonable, pero el resto tiene que estar presentado arriba.}
...
{\huge Lo del corte del vinculo quimico queda medio solitario, expandi un poco la explicacion para que se entienda porque es un problema, quizas junto con la figura que aun tenes que poner, se llegue a captar bien la idea.}
{\huge La figura sacada del cramer sigue siendo muy chota, mejorala y agrega a la explicacion que lo que se esta representando es, de alguna manera, la parte del sistema que es resuelta mediante QM y MM.}
...
{\Huge Esta parte TIENE que ir en la introduccion teorica. No solo la cuenta de las energias, toda la explicacion de como se llega a lo que tenes que calcular tiene que ir ahi.}
...
{\Huge Tenes que poner que significa cada uno de las variables que introducis. Una formula donde  no decis que respresenta cada termino, no dice nada (ademas de tener que ir en la introduccion.}
...
{\Huge Idem arriba. Podes ponerle un label a la ecuacion y hacerle referencia aqui, pero esto BIEN EXPLICADO tiene que ir en la introduccion. Si queres dejar aca la parte del costo de calcularlo, me parece razonable, pero el resto tiene que estar presentado arriba.}
...
% \end{center}
\caption[Tiempo consumido por las rutinas]{\small{Tiempo consumido por las rutinas.} {\Huge Saca los titulos que forman parte de los graficos, no quedan bien.}}
\label{fig:tiempos}
\end{figure}
...
\caption[Porcentajes de tiempo consumidos por las rutinas]{\small{Porcentajes de tiempo consumidos por las rutinas.}{\Huge Saca los titulos que forman parte de los graficos, no quedan bien.}}
\label{fig:Profiling}
\end{figure}
...
{\Huge Si vas a hacer referencia a RMM, tenes que decir que es(tanto en el caption como en el texto donde presentas la figura). Tambien indica que significan las distintas flechas (quizas con una leyenda sobre el mismo grafico}
...
Se utilizaron las librer\'ias \texttt{MPICH}\cite{MPICH} y \texttt{LAM MPI}\cite{LAM_MPI} para las pruebas realizadas.
{\Huge cualquiera con dos dedos de frente se va a preguntar porque usaste las dos o si todas las corridas las hiciste con las dos y si hubo diferencias en tiempos. Pone en que caso usaste cada una y porque.}
...
Para resolver finalmente el problema de precisi\'on en punto flotante se intent\'o realizar un reordenamiento de los c\'alculos para que el orden de las operaciones en la sumatoria {\huge que sumatoria? pone una referencia a la ecuacion, o parte de ella, a la que te estas refiriendo} sea similar al del programa original.
...
Finalmente de la observaci\'on de los datos se lleg\'o a una soluci\'on alternativa al problema. Si bien los resultados obtenidos no eran iguales a los del serial, los mismos eran muy similares, y a los efectos de la investigaci\'on qu\'imica para la cual se utilizar\'ian, eran v\'alidos {\Huge esto no se entiende a que va? Estas hablando de la solucion que vas a plantear en el proximo parrafo, o que?}.
...
{\huge Que es esto que esta en negrita? No se entiende de que va. Si queres aclarar a que corresponde cada corrida, podes poner una seccion donde los explicas y donde mostrar un diagrama de la estructura quimica del mismo, estaria bueno.}
...
Una vez implementado el algoritmo con precisi\'on exendida se realizaron corridas en una m\'aquina SMP Xeon Dual, obteni\'endose los resultados de la tabla \ref{tab:TiemposEjecucion1}. {\Huge podes hacer referencia a la pagina o seccion donde hablabas de esta modificacion para que no quede tan descolgado, si vas a hablar inicialmente de la solucion de precision extendida, podes ponerle ese titulo.}
...
{\huge en todos los graficos tenes que indicar a que corresponde la corrida y que estas mostrando. Se tiene que entender sin leer el texto.}
